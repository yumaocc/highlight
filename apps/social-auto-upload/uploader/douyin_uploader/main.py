...
DOUYIN_UPLOAD_COMPLETE_TIMEOUT_MS = int(os.getenv("DOUYIN_UPLOAD_COMPLETE_TIMEOUT_MS", "300000"))
DOUYIN_PUBLISH_READY_TIMEOUT_MS = int(os.getenv("DOUYIN_PUBLISH_READY_TIMEOUT_MS", "180000"))
DOUYIN_PUBLISH_SUBMIT_TIMEOUT_MS = int(os.getenv("DOUYIN_PUBLISH_SUBMIT_TIMEOUT_MS", "300000"))
DOUYIN_DETECTION_GRACE_MS = int(os.getenv("DOUYIN_DETECTION_GRACE_MS", "15000"))
...
class DouYinBaseUploader(BaseVideoUploader):
    ...
    async def set_self_declaration(self, page: Page, declaration: str = "内容为个人观点或见解") -> bool:
        """抖音「自主声明」为发布必选项：打开声明弹窗 → 选指定类型 → 确定。

        入口和弹窗都是异步渲染，等不到时返回 False，由发布流程继续重试并输出诊断。
        """
        try:
            await self.dismiss_publish_overlays(page)
    ...
    async def dismiss_publish_overlays(self, page: Page) -> None:
        """清理抖音发布页常见引导浮层，避免拦截按钮点击。"""
        try:
            for _ in range(4):
                clicked = False
                know_buttons = page.get_by_role("button", name="我知道了")
                for index in range(await know_buttons.count()):
                    button = know_buttons.nth(index)
                    try:
...
class DouYinVideo(DouYinBaseUploader):
    ...
    async def collect_publish_diagnostics(self, page: Page, reason: str) -> Path:
        diagnostics_dir = BASE_DIR / "logs" / "douyin_diagnostics"
        diagnostics_dir.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        safe_reason = "".join(ch if ch.isalnum() or ch in "-_" else "_" for ch in reason)[:40]
        base_path = diagnostics_dir / f"{timestamp}_{safe_reason}"

        try:
            await page.screenshot(path=str(base_path.with_suffix(".png")), full_page=True)
        except Exception as exc:
            douyin_logger.warning(_msg("😵", f"诊断截图保存失败: {exc}"))

        try:
            body_text = await page.locator("body").inner_text(timeout=3000)
            base_path.with_suffix(".txt").write_text(body_text, encoding="utf-8")
        except Exception as exc:
            douyin_logger.warning(_msg("😵", f"诊断文本保存失败: {exc}"))

        try:
            base_path.with_suffix(".html").write_text(await page.content(), encoding="utf-8")
        except Exception as exc:
            douyin_logger.warning(_msg("😵", f"诊断 HTML 保存失败: {exc}"))

        return base_path

    async def get_visible_publish_blockers(self, page: Page) -> list[str]:
        markers = [
            "上传失败",
            "视频不符合",
            "作品时长过短",
    ...
    async def wait_for_video_upload_complete(self, page: Page) -> None:
        deadline = asyncio.get_running_loop().time() + DOUYIN_UPLOAD_COMPLETE_TIMEOUT_MS / 1000
        while asyncio.get_running_loop().time() < deadline:
            try:
                upload_failed = await page.locator('div.progress-div > div:has-text("上传失败")').count()
                if upload_failed or await page.get_by_text("上传失败").count():
                    douyin_logger.error(_msg("😵", "检测到上传失败，小人准备重试"))
                    await self.handle_upload_error(page)
                    await asyncio.sleep(2)
                    continue

                # 2026-06 version_2：上传完成后预览卡片露出“重新上传”按钮；旧版 long-card 选择器仍保留兼容。
                old_done = await page.locator('[class^="long-card"] div:has-text("重新上传")').count()
                new_done = await page.get_by_text("重新上传", exact=True).count()
        ...
        diagnostics_path = await self.collect_publish_diagnostics(page, "upload_timeout")
        blockers = await self.get_visible_publish_blockers(page)
        raise TimeoutError(f"等待抖音视频上传完成超时，页面线索: {blockers}，诊断文件: {diagnostics_path}")

    async def wait_for_publish_ready(self, page: Page) -> None:
        deadline = asyncio.get_running_loop().time() + DOUYIN_PUBLISH_READY_TIMEOUT_MS / 1000
        detection_grace_deadline = asyncio.get_running_loop().time() + DOUYIN_DETECTION_GRACE_MS / 1000
        ...
        while asyncio.get_running_loop().time() < deadline:
            ...
            # “横/竖双封面缺失”是流量建议，不是发布硬阻塞；没有提供封面时允许继续。
            hard_blockers = [
                blocker for blocker in blockers
                if blocker not in {"横/竖双封面缺失", "检测中"}
            ]
            detection_still_running = "检测中" in blockers and asyncio.get_running_loop().time() < detection_grace_deadline
            if not hard_blockers and not detection_still_running:
                return

            douyin_logger.info(_msg("🧭", f"发布前检查还没完成，页面线索: {blockers}"))
            await asyncio.sleep(3)

        diagnostics_path = await self.collect_publish_diagnostics(page, "publish_not_ready")
        raise TimeoutError(f"等待抖音发布页就绪超时，页面线索: {last_blockers}，诊断文件: {diagnostics_path}")

    async def submit_publish(self, page: Page) -> None:
        deadline = asyncio.get_running_loop().time() + DOUYIN_PUBLISH_SUBMIT_TIMEOUT_MS / 1000
        last_blockers: list[str] = []
        while asyncio.get_running_loop().time() < deadline:
            try:
                await self.dismiss_publish_overlays(page)
                ...
                last_blockers = await self.get_visible_publish_blockers(page)
                douyin_logger.info(_msg("🏃", f"小人正在冲刺发布视频，页面线索: {last_blockers}"))
                await asyncio.sleep(1)

        diagnostics_path = await self.collect_publish_diagnostics(page, "publish_timeout")
        raise TimeoutError(f"提交抖音发布超时，页面线索: {last_blockers}，诊断文件: {diagnostics_path}")
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        await self.wait_for_video_upload_complete(page)
        ...
        await self.wait_for_publish_ready(page)
